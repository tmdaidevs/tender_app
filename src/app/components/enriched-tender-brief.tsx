...
function FactList({
  ...
  items: Array<{ text: string; sourcePath: string }>;
}) {
  if (items.length === 0) {
  ...
  return (
    <section className="enrichment">
      <div className="ai-brief-banner">
        <span className="ai-brief-icon"><Sparkles size={20} /></span>
        <div>
          <small>AI-ORGANIZED · OFFICIAL SOURCE EVIDENCE</small>
          <h2>Opportunity brief</h2>
          <p>
            OpenAI structured this brief from the complete official notice. Every factual
            bullet retains its source element; always verify the official notice.
          </p>
        </div>
        <div className="ai-brief-meta">
          <strong>{enrichment.model.replace("openai/", "")}</strong>
          ...
          }).format(new Date(enrichment.completedAt))}</span>
        </div>
      </div>
      ...
      <div className="enrichment-layout">
        ...
        <section className="enrichment-card">
          ...
          <small className="source-paths">{brief.buyer.sourcePaths.join(" · ")}</small>
        </section>
        ...
        <section className="enrichment-card">
          ...
          <small className="source-paths">{brief.procedure.sourcePaths.join(" · ")}</small>
        </section>
        ...
        <section className="enrichment-card">
          ...
          <small className="source-paths">{brief.commercial.sourcePaths.join(" · ")}</small>
        </section>
      ...
      <footer className="enrichment-provenance">
        <div>
          <strong>Generation provenance</strong>
          <span>
            Prompt {enrichment.promptVersion} · {enrichment.totalTokens?.toLocaleString("en-GB") ?? "—"} tokens
          </span>
        </div>
        <a href={enrichment.sourceDocumentUrl} target="_blank" rel="noreferrer">
          Open official source XML <ExternalLink size={13} />
        </a>
      </footer>
    </section>
  );
